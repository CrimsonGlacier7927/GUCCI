...
### ۳. اولین ورود به پنل
آدرس پنل شما این خواهد بود (پنل **فقط** با همین مسیر باز می‌شود):
```
...
> 🔒 **حریم خصوصی:** اگر کسی فقط خودِ دامنه را باز کند (مثلاً `https://دامنه‌شما.up.railway.app`)، به‌جای پنل یک **صفحه کاملاً سیاه** می‌بیند. مسیر قبلی `/managepanel/` هم دیگر کار نمی‌کند و پنل فقط از طریق `/gucci/` در دسترس است.
